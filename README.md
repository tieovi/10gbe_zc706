...
| `stream.cfg` | litex_server streaming config |
| `analyzer.csv` | LiteScope analyzer signal list |
| `csr.csv` | CSR register map (generated by last build) |
| `tight_setup_hold_pins.txt` | Timing-critical pin list for XDC constraints |
| `fix_xdc_constraints.py` | Utility to patch timing constraints in the XDC |
...
This project uses a patched LiteEth with XGMII support:
https://github.com/enjoy-digital/liteeth/pull/88
